...
{
	printk(KERN_INFO "IN SFS GET BLOCK");
	...
	if (iblock >= SIMULA_FS_DATA_BLOCK_CNT /* Compare with the max data block count we support in SFS design */)
	{
		return -ENOSPC;
	}
	...
	if (!fe.blocks[iblock])
	{
		...
		{
			if ((fe.blocks[iblock] = sfs_get_data_block(info) /*Get a free block for data block from SFS */) == INV_BLOCK)
			{
				return -ENOSPC;
			}
	...
	phys = fe.blocks[iblock] * info->sb.block_size / block_size(sb->s_bdev) /* Translate the SFS block number fe.blocks[iblock] to block driver block number */;
	printk(KERN_ERR "KERNEL FE BLOCK %d \n",fe.blocks[iblock]);
	printk(KERN_ERR "KERNEL SB BLOCK SIZE %d \n",block_size(sb->s_bdev));
	printk(KERN_ERR "KERENEL PHYSICAL BLOCK %d \n",phys);
	printk(KERN_ERR "$$$$$$$$");
...
static int sfs_readpage(struct file *file, struct page *page)
{
	...
	return mpage_readpage(page, sfs_get_block /* Callback function to get the block number of the desired data block */);
}
	...
	loff_t pos, unsigned len, unsigned flags, struct page **pagep, void **fsdata)
{
	...
	*pagep = NULL;
	...
	return block_write_begin(file, mapping, pos, len, flags, pagep, fsdata,
		sfs_get_block);
#else
	return block_write_begin(mapping, pos, len, flags, pagep, sfs_get_block /*  Callback function to get the block number of the desired data block */);
#endif
}
static int sfs_writepage(struct page *page, struct writeback_control *wbc)
{
	printk(KERN_INFO "sfs: sfs_writepage\n");
	return block_write_full_page(page, sfs_get_block /*  Callback function to get the block number of the desired data block */, wbc);
}
static struct address_space_operations sfs_aops =
{
...
static int sfs_readdir(struct file *file, void *dirent, filldir_t filldir)
{
	...
	return sfs_list(NULL, NULL, NULL, NULL); /* Fill in all the parameters */
}
#else
static int sfs_iterate(struct file *file, struct dir_context *ctx)
{
	...
	return sfs_list(info, file, ctx); /*  Fill in all the parameters */
}
#endif
static struct file_operations sfs_fops =
{
...
#endif
{
	printk(KERN_INFO "sfs: This is sfs inode lookup");
	...
	if ((ino = sfs_lookup(info, fn, &fe)) == INV_INODE) /* Fill in all the parameters */
	...
	if (file_inode->i_state & I_NEW)
	{
		...
		file_inode->i_atime.tv_sec = file_inode->i_mtime.tv_sec = file_inode->i_ctime.tv_sec = fe.timestamp /* : SFS file timestamp */;
...
static int   sfs_inode_create(struct inode *parent_inode, struct dentry *dentry, umode_t  mode, bool excl)
{
	...
	if ((ino = sfs_create(info,fn, perms, &fe)) == INV_INODE) /*  Fill in all the parameters */
		return -ENOSPC;
...
static int sfs_inode_unlink(struct inode *parent_inode, struct dentry *dentry)
{
	...
	strncpy(fn, dentry->d_name.name, SIMULA_FS_FILENAME_LEN);
	fn[SIMULA_FS_FILENAME_LEN] = 0;
	if ((ino = sfs_remove(info, fn)) == INV_INODE) /* Fill in all the parameters */
		return -EINVAL;
...
static int sfs_inode_rename(struct inode *old_dir, struct dentry *old_dentry,
			struct inode *new_dir, struct dentry *new_dentry, unsigned int flags){
		sfs_info_t *info=(sfs_info_t *)(old_dir->i_sb->s_fs_info);
		char old_file_name[SIMULA_FS_FILENAME_LEN+1];
		char new_file_name[SIMULA_FS_FILENAME_LEN+1];
		printk(KERN_INFO "SFS FILE RENAME");
		strncpy(old_file_name,old_dentry->d_name.name,SIMULA_FS_FILENAME_LEN);
		old_file_name[SIMULA_FS_FILENAME_LEN]=0;
		strncpy(new_file_name,new_dentry->d_name.name,SIMULA_FS_FILENAME_LEN);
		new_file_name[SIMULA_FS_FILENAME_LEN]=0;
		return sfs_rename(info,old_file_name,new_file_name);
...
{
	sfs_info_t *info = (sfs_info_t *)(sb->s_fs_info /*: Get the private data from the VFS super block */);
...
#endif
{
	...
	perms = 0;
	perms |= (inode->i_mode & (S_IRUSR | S_IRGRP | S_IROTH)) ? 4 /*  SFS permission for read */ : 0;
	perms |= (inode->i_mode & (S_IWUSR | S_IWGRP | S_IWOTH)) ? 2 /*: SFS permission for write */ : 0;
	perms |= (inode->i_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) ? 1 /* : SFS permission for execute */ : 0;

	printk(KERN_INFO "sfs: sfs_write_inode with %d bytes @ %d secs w/ %o\n",
		size, timestamp, perms);

	return sfs_update(info, inode->i_ino, &size, &perms,&timestamp); /*  Fill in all the parameters */
}
static int sfs_fetch_stats(struct dentry *dentry,struct kstatfs *buf){
	sfs_info_t *info=(sfs_info_t *)(dentry->d_sb->s_fs_info);
	printk(KERN_INFO "fetching filesystem stats");
	return get_fs_stats(info,buf);
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Deepanshu kartikey <Kartikey406@gmail.com>");
